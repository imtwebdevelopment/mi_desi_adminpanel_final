...
const OrderPage = ({ onNavigate }) => {
  ...
  const handleAcceptOrder = async (order) => {
    try {
      ...
      sendPushNotification({
        customerId: order.customerId,
        fcmToken: order.customerFcmToken || null,
        title: "Order Shipped",
        body: `Your order #${order.id} has been accepted and shipped!`,
        screen: "OrderDetails",
      });
  ...
  const handleStatusChange = async (docId, customerId, newStatus) => {
    try {
      ...
      // Send push notification to the customer
      const currentOrder = orders.find(o => o.docId === docId);
      sendPushNotification({
        customerId: customerId,
        fcmToken: currentOrder ? currentOrder.customerFcmToken : null,
        title: `Order Update`,
        body: `Your order #${currentOrder?.id || docId} is now ${newStatus}.`,
        screen: "OrderDetails",
      });
