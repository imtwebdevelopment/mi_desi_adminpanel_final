...
const UnifiedRechargeRequestList = ({ role = "admin" }) => {
        ...
        return reqSnap.docs
          .map((reqDoc) => {
            ...
            return {
              ...
              customerFcmToken: customerLookup[userId]?.fcmToken || "",
              ...data,
            };
          })
          .filter(Boolean);
  ...
  const handleStatusChange = async (userId, requestId, newStatus) => {
    ...
    try {
      ...
      if (newStatus === "Success") {
        const currentReq = requests.find(r => r.id === requestId && r.userId === userId);
        sendPushNotification({
          customerId: userId,
          fcmToken: currentReq?.customerFcmToken || null,
          title: "Recharge Approved",
          body: `Your recharge request of ₹${currentReq?.plan?.price || ""} was successful!`,
          screen: "RechargeRequest",
        });
      }
  ...
  const confirmReject = async () => {
    ...
    try {
      ...
      const currentReq = requests.find(r => r.id === rejectModal.requestId && r.userId === rejectModal.userId);
      sendPushNotification({
        customerId: rejectModal.userId,
        fcmToken: currentReq?.customerFcmToken || null,
        title: "Recharge Rejected",
        body: `Your recharge request was rejected: ${rejectReason}`,
        screen: "RechargeRequest",
      });
